...
def map_to_emotion(valence_score, arousal_score, intensity):
    ...
    Note: Since original values are negative, lower values actually indicate higher intensity
    """
    # For negative values, lower (more negative) means stronger emotion
    v_threshold = 0
    a_threshold = 0
    i_threshold = np.median([abs(valence_score), abs(arousal_score)])

    if valence_score < v_threshold:  # Changed from > to <
        if arousal_score < a_threshold:  # Changed from > to <
            # High negative valence, high negative arousal
            return 0 if intensity < -i_threshold else 7  # happy vs hopeful
        else:
            # High negative valence, low negative arousal
            return 5 if intensity < -i_threshold else 3  # peaceful vs love
    else:
        if arousal_score < a_threshold:  # Changed from > to <
            # Low negative valence, high negative arousal
            return 2 if intensity < -i_threshold else 4  # angry vs fear
        else:
            # Low negative valence, low negative arousal
            return 1 if intensity < -i_threshold else 6  # sad vs nostalgic


def compute_emotional_profile(features):
    """Compute emotional profile using scaled features with inverted logic"""

    # Calculate composite scores (more negative = stronger emotion)
    valence_score = (
        features["valence_mean"] * 0.2
        + features["valence_median"] * 0.15
        + features["valence_skewness"] * -0.15  # Inverted for negative values
        + features["valence_kurtosis"] * -0.15  # Inverted for negative values
        + features["valence_variation"] * 0.15
        + (features["valence_min"] - features["valence_max"]) * 0.2  # Inverted max/min
    )

    arousal_score = (
        features["arousal_mean"] * 0.2
        + features["arousal_median"] * 0.15
        + features["arousal_skewness"] * -0.15  # Inverted for negative values
        + features["arousal_kurtosis"] * -0.15  # Inverted for negative values
        + features["arousal_variation"] * 0.15
        + (features["arousal_min"] - features["arousal_max"]) * 0.2  # Inverted max/min
    )

    # Compute intensity (more negative = more intense)
    intensity = (
        -(abs(features["valence_quartile1"] - features["valence_quartile3"])) * 0.3
        + -(abs(features["arousal_quartile1"] - features["arousal_quartile3"])) * 0.3
